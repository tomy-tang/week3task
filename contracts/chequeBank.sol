...
contract ChequeBank {
    ...
    function _removeCheque(uint _id) private {
        bytes32 chequeId;
        uint last = chequeList.length - 1;
        for (uint i = _id; i < last; i++) {
    ...
    function pureCheque() external {
        ...
        for (uint i = 0; i <= last; i++) {
            ...
            expire =
                expire ||
                cheques[chequeList[i]].chequeInfo.payer == address(0);
            if (current >= 100) {
                break;
            }
            if (expire) {
    ...
    function redeem(Cheque memory chequeData) external {
        ...
        require(
            available >= amount,
            "insufficient balance on issuer, can not redeem, contact cheque issuer to deposit more"
        );
    ...
    function isToValid(uint to, uint current) private pure returns (bool) {
     ...
     function _validate(bytes32 chequeId)
        private
        view
        returns (
            bool good,
        ...
        )
    {
		...
		// balance should be enough to pay
		if (availables[cheque.payer] < cheque.amount){
			return (good, clean,payee);
		}
    ...
    function chequeHash(ChequeInfo calldata _info)
        private
        view
        returns (bytes32)
    {
    ...
    function signatureCheque(ChequeInfo calldata _info, bytes memory sig)
        external
    {
        Cheque storage cheque = cheques[_info.chequeId];
        require(
            cheque.chequeInfo.payer == address(0),
            "can not sign a signed cheque"
        );
    ...
    function isChequeValid(address payee, bytes32 chequeId)
        public
        view
        returns (bool)
    {
